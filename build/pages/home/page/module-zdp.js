...
KISSY.add('page/module-zdp', function(S, Node, DOM, IO, XTemplate, Countdown, Time) {

	var $ = Node.all;

	function discount() {
		var counts = $('#zdp-goods-box .item');

		counts.each(function(item, index) {

			var el = item.all('.time-tips');
	...
	function templateExt( data ) {
		//to fixed 2
       	XTemplate.addCommand('toFixed', function(scopes, option) {

       		var num = new Number(option.params[0]),
  				fixNum = option.params[1] || 2;
			return num.toFixed(fixNum);
       	});

		//to discount
       	XTemplate.addCommand('discount', function(scopes, option) {

       		var ends = option.params[0]*1,
       			leftTime = 0;

       		if ( !data.systemTime ) {
       			data.systemTime = Time.getNow();
       		}

       		var remainTime = Time.getLeftTime(ends, data.systemTime);
       		if ( remainTime == -1 ) {
       			leftTime = 0;
       		} else {
       			leftTime = Time.getHours(remainTime)*3600+Time.getMin(remainTime)*60+Time.getSec(remainTime);
       		}

       		//%u5269%u4F59%u65F6%u95F4%u5206%u79D2
			var html = '';

			html += '<p class="time-tips" data-config="{\'leftTime\':' + leftTime + '}">';

			html += '\u5269\u4F59\u65F6\u95F4 : ${h} \u65F6 ${m} \u5206 ${s} \u79D2';
	...
	function render(data) {
		var tpl = $('#tpl-zdp-goods'),
			tplTo = $('#zdp-goods-box');

		if ( !data ) {
			throw new Error('no data from '+JSON_URL_ZDPAI);
		}

		var sysTime = data.systemTime || 0;
		var data_clothesList = data.clothesList || [],
			data_shoeBagList = data.shoeBagList || [],
			data_ornamentList = data.ornamentList || [],
			data_homeList = data.homeList || [];
		...
		function build( list ) {
			...
			_data.data = list;
			if ( list.length > 0 ) {
				var render = new XTemplate(tpl.html()).render(_data);
  				tplTo.append(render);
			}
		} 

		//build the tpl
		build(data_clothesList);
		build(data_shoeBagList);
		build(data_ornamentList);
		build(data_homeList);

		var first = $('#zdp-goods-box .zdp-goods-list')[0];
		$(first).addClass('no-border');
	...
	function getData() {
		IO.jsonp(JSON_URL_ZDPAI, function(data) {
			render(data);
		});	
	}

	function init() {
		//show depend on content show
		if ( $('#zdp-content')[0] ) {
			getData();
		}
	}
	...
	requires: ['node', 'dom', 'ajax', 'xtemplate', 'widget/countdown/index', 'widget/time/index']
});
